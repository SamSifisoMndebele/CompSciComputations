...
class PublicServiceImpl : PublicService {
    ...
    override suspend fun getOnboardingItem(id: Int): OnboardingItem? = dbQuery(conn) {
        executeQuerySingleOrNull("select * from public.sql.onboarding_items where id = ?", { getOnboardingItem() }) {
            setInt(1, id)
        }
    }

    override suspend fun getOnboardingItems(): List<OnboardingItem> = dbQuery(conn) {
        executeQuery("select * from public.sql.onboarding_items", { getOnboardingItem() })
    }

    override suspend fun getOnboardingItemsExcept(ids: IntArray): List<OnboardingItem> = dbQuery(conn) {
        executeQuery("select * from public.sql.onboarding_items where id not in (${ids.joinToString()})",
            { getOnboardingItem() })
    }

    override suspend fun deleteOnboardingItems(id: Int): Unit = dbQuery(conn) {
        executeUpdate("delete from public.sql.onboarding_items where id = ?") {
            setInt(1, id)
        }
    }
}