...
object AuthDataStore {
    ...
    val Context.firstLaunchFlow: Flow<Boolean>
        get() = dataStore.data.map { preferences ->
            preferences[com.compscicomputations.client.auth.AuthDataStore.firstLaunchKey] ?: true
        }
    suspend fun Context.setFirstLaunch(isFirstLaunch: Boolean) = dataStore.edit { preferences ->
        preferences[com.compscicomputations.client.auth.AuthDataStore.firstLaunchKey] = isFirstLaunch
    }

    val Context.termsAcceptedFlow: Flow<Boolean>
        get() = dataStore.data.map { preferences ->
            preferences[com.compscicomputations.client.auth.AuthDataStore.termsAcceptedKey] ?: false
        }
    suspend fun Context.setTermsAccepted(isAccepted: Boolean) = dataStore.edit { preferences ->
        preferences[com.compscicomputations.client.auth.AuthDataStore.termsAcceptedKey] = isAccepted
    }
