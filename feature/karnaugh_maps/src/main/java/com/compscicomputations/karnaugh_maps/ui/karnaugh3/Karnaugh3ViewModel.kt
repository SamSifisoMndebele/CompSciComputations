...
class Karnaugh3ViewModel(
    ...
    init {
        viewModelScope.launch {
            ThreeVarsDataStore.lastState(context).first()
                ?.let { state ->
    ...
    fun clear() { _uiState.value = Karnaugh3UiState(convertFrom = _uiState.value.convertFrom) }
    ...
    fun onExpressionChange(textFieldValue: TextFieldValue) {
        if (textFieldValue.text.isBlank()) {
            _uiState.value = Karnaugh3UiState(convertFrom = _uiState.value.convertFrom)
            return
        }
        ...
        val answers = Karnaugh3Variables(minTerms,  IntArray(0)).executeKarnaugh()

        _uiState.value = _uiState.value.copy(
            minTerms = minTerms,
            answers = answers.ifEmpty { listOf(ListOfMinTerms(0)) },
            selected = 0,
    ...
    companion object {
        const val TAG = "Karnaugh3ViewModel"
    }
}